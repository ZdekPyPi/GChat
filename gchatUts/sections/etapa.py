...
from gchat.uikit import UiSection, UiDecoratedText, UiColumn, UiColumns
...
class SectionEtapa:
    ...
    def render_etapas(self):
        ...
        col_1 = UiColumn(widgets=[UiDecoratedText(text=self.title)])
        col_2 = UiColumn(
            widgets=[UiDecoratedText(text=" ".join([x.icone for x in self.etapas]))]
        )
        ...
        for desc, job_etapas in self.jobs.items():
            ...
            for etapa in self.etapas:
                icn = [
                    x.status for x in job_etapas if x.id_etapa == etapa.id_etapa
                ] or [ItemEtapa.STS_NOIC]
                icn_etapas.append(icn[0])

            col_2.widgets.append(UiDecoratedText(text=" ".join(icn_etapas)))

        return UiSection(
            header="Status por Etapa", widgets=[UiColumns(columns=[col_1, col_2])]
        ).render()
