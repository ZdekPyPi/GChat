...
class UiDecoratedText:
    def __init__(self,top_label=None,text=None,bottom_label=None,icon_url=None,right_button:UiButton=None):
...
class UiColumn:
    ...
    def render(self):
        base = {"horizontalAlignment": self.horizontal_alignment,"widgets": [x.render() for x in self.widgets]}
        return base
...
class UiCard:
    def __init__(self,title=None,subtitle=None,image_url=None,sections:list[UiSection]=[]):
        self.title     = title
    ...
    def render(self):
        ...
        encoded_image_url = quote(self.image_url, safe=':/?#[]@!$&\'()*+,;=') if self.image_url else None
        return { "card": { "header": { "title": self.title, "subtitle": self.subtitle, "imageUrl": encoded_image_url }, "sections": sections } }